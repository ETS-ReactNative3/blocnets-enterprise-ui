...
export function catalogue(chaincode, key) {
    return async (dispatch) => {
        ...
        await axios.head(config.chaincodes.Default + config.chaincodes.CAT + chaincode, key, { headers })   // Check if chaincode Key exists
            .then(async () => {                                                                                   // If chaincode Key does exist.. continue
                ...
                await axios.get(config.chaincodes.Default + config.chaincodes.CAT + chaincode, { headers }) // GET existing data by chaincode key
                    .then(async (response) => {
                        ...
                        console.log(keys);
                        await axios.put(config.chaincodes.Default + config.chaincodes.CAT + chaincode, keys, { headers })  // UPDATE on the existing Key with the new array of keys
                            .then(async () => {
                                return dispatch({
            ...
            .catch(async () => {                                                                                  // If chaincode Key does not exist.. continue
                dispatch({
                    type: "CHECKED_CAT_KEY_DOES_NOT_EXIST",
                    payload: true
                });
                await axios.post(config.chaincodes.Default + config.chaincodes.CAT + chaincode, key, { headers }) // POST data and new chaincode key
                    .then(() => {
                        return dispatch({
